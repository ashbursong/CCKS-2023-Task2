...
def train_and_validate(cfg, model, train_data, valid_data, filtered_data=None):
    ...
    for i in range(0, cfg.train.num_epoch, step):
        ...
        if result > best_result:
            ...
            logger.warning("better model found at epoch %d, Save it!" % epoch)
    util.synchronize()
            ...
            if metric == "mr":
                ...
                if len(values) > 1:
                    ...
                    for i in range(num_sample):
                        ...
                        num_comb = math.factorial(num_sample - 1) / \
                                   math.factorial(i) / math.factorial(num_sample - i - 1)
                        score += (1 / (i + 1)) * num_comb * (fp_rate ** i) * ((1 - fp_rate) ** (num_sample - i - 1))
                    score = score.mean()
                else:
...
if __name__ == "__main__":
    ...
    is_inductive = cfg.dataset["class"].startswith("Ind") or cfg.dataset['class'] == "CCKS"
