...
class IndRelLinkPredDataset(InMemoryDataset):
    ...
    def __init__(self, root, name, version, transform=None, pre_transform=None):
        ...
        assert name in ["FB15k-237", "WN18RR", "NELL"]
...
class CCKS(InMemoryDataset):
    ...
    def process(self):
        self.data_dir = "/home/slyang/tyliu_code/NBFNet/data/CCKS"
        train_file = os.path.join(self.data_dir, "train.txt")
        valid_file = os.path.join(self.data_dir, "test_support.txt")
        ...
        train_fact_slice = slice(None, sum(num_samples[:1]))
        valid_fact_slice = slice(sum(num_samples[:1]), sum(num_samples[:2]))
        
        train_fact_index = edge_index[:, train_fact_slice]
        train_fact_type = edge_type[train_fact_slice]
        valid_fact_index = edge_index[:, valid_fact_slice]
        valid_fact_type = edge_type[valid_fact_slice]
        # add flipped triplets for the fact graphs
        train_fact_index = torch.cat([train_fact_index, train_fact_index.flip(0)], dim=-1)
        train_fact_type = torch.cat([train_fact_type, train_fact_type + num_relations])
        
        valid_fact_index = torch.cat([valid_fact_index, valid_fact_index.flip(0)], dim=-1)
        valid_fact_type = torch.cat([valid_fact_type, valid_fact_type + num_relations])
        ...
        train_data = Data(edge_index=train_fact_index, edge_type=train_fact_type, num_nodes=len(inv_train_entity_vocab),
                          target_edge_index=edge_index[:, train_slice], target_edge_type=edge_type[train_slice])
        valid_data = Data(edge_index=valid_fact_index, edge_type=valid_fact_type, num_nodes=len(inv_test_entity_vocab),
                         target_edge_index=edge_index[:, valid_slice], target_edge_type=edge_type[valid_slice])
        ...
        torch.save((self.collate([train_data, valid_data])), self.processed_paths[0])
