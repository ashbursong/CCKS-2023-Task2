...
def create_working_directory(cfg):
    ...
    working_dir = os.path.join(os.path.expanduser(cfg.output_dir),
                               cfg.model["class"], cfg.dataset["class"], cfg.exp_name)
...
def build_dataset(cfg):
    ...
    if cls == "FB15k-237":
        ...
        dataset = datasets.BDSC(**cfg.dataset)
        dataset.num_relations = (int(dataset.data.edge_type.max()) + 1) * 2
    elif cls == "CCKS":
        dataset = datasets.CCKS(**cfg.dataset)
    ...
    if get_rank() == 0:
        ...
        logger.warning("#train: %d, #valid: %d" %
